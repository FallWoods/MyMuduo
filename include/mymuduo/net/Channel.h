...
// 用来监听发生在fd上的事件，事件包括空事件（不监听）、可读事件、可写事件。
...
public:
    ...
    //设置事件回调，由Channel对象持有者即TcpConnection处理Channel事件时回调
...
private:
    ...
    int revents_;      //实际发生的事件，Poller类设置
    ...
    EventCallback closeCallback_;     /* 连接关闭事件回调 */
};