...
public:
    ...
    //设置用于新建连接的回调，通过此函数建立新的TcpConnection对象
...
private:
    ...
    Socket acceptSocket_;//专门用于接收连接的套接字
    Channel acceptChannel_;//绑定上述套接字的通道，专门用于接收连接
    NewConnectionCallback newConnectionCallback_;  //建立新连接的回调，每当接收一个客户端的连接请求建立新连接后，就调用此函数
